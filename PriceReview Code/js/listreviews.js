...
let reportReiewUrl = './src/reportReview.php';
...
function listreviews(id) {
    ...
    try {
        request.onreadystatechange = function () {
            ...
            var reportButton = `
                    <button type="button" id="reportBtn${i}" class="btn btn-default" onclick="reportReview(${i});">
                    Report this post!
                    </button>`;

    var row = `<li><h2>${count}</h2><h3>Review ${jsonArray[i].review_id} <hr> Product Name: ${jsonArray[i].product_name}</h3><p><br><br>Review Description:<br><br>${jsonArray[i].review_description}</p>${readMoreButton}${reportButton}</li>`;
...
function reportReview(i)
{
    ...
    var request = new XMLHttpRequest();
    request.open("POST", reportReiewUrl, true);
    request.setRequestHeader("Content-type", "application/json; charset=UTF-8");
    try {
        request.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
               jsonArray = JSON.parse(request.responseText);
                ...
                if (jsonArray.msg === "No Contacts Found")
                    ...
                    window.location.href = "./listofreviews.html";
